...
def build_dataset(is_train, args):
    ...
    if entry['type']=='imagefolder':
        if args.dataset == "flowers":
            dataset = Flowers(root=entry['path'], transform=transform, istrain=is_train)
                ...
                dataset = CUBDataset(root=entry['path'], transform=transform, train=is_train)
        else:
    ...
    # ## ------------------------- scale ---------------------------------------------
    # if is_train:
    #     dataset = generate_few_shot_flowers(dataset, args.shot)    
    return dataset, len_original

def generate_few_shot_flowers(dataset, num_shot):
    if num_shot < 0: 
        return dataset
    class_indices = defaultdict(list)
    for i, element in enumerate(dataset.data):
        class_indices[element[1]].append(i)
    selected_indices = []
    for indices in class_indices.values():
        # Check if k is greater than the number of elements in the class
        if len(indices) * num_shot < 1:
            print(f"Warning: Class has fewer than {num_shot * 100}% elements.")
            selected_indices.extend(indices)
        else:
            num_to_select = int(len(indices) * num_shot)
